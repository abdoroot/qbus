    ...
    <table id="destinations-table" class="table display table-bordered table-striped no-wrap">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if(!is_null($destination->fromCity)) {{ $destination->fromCity->name }} @endif    
                </td>
                <td>
                    @if(!is_null($destination->toCity)) {{ $destination->toCity->name }} @endif    
                </td>
                <td>
                    @if(!is_null($destination->startingTerminal))
                    <a href="{{ route('provider.terminals.show', $destination->starting_terminal_id) }}" class="text-primary"> {{ $destination->startingTerminal->name }}</a>
                    @endif    
                </td>
                <td>
                    @if(!is_null($destination->arrivalTerminal))
                    <a href="{{ route('provider.terminals.show', $destination->arrival_terminal_id) }}" class="text-primary"> {{ $destination->arrivalTerminal->name }}</a>
                    @endif    
                </td>
                <td>{{ count($destination->stops ?? []) }}</td>
                <td>
                    <a href="{{ route('provider.destinations.edit', [$destination->id]) }}"
                        class='btn btn-info btn-sm'>
                            <i class="ti-pencil"></i>
                        </a>
                    {!! Form::open(['route' => ['provider.destinations.destroy', $destination->id], 'method' => 'delete', 'class' => 'd-inline']) !!}
