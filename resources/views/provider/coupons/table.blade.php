<div class="table-responsive m-t-30 m-b-20">
    <table id="coupons-table" class="table display table-bordered table-striped no-wrap">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $coupon->id }}</td>
                <td><a href="{{ route('provider.coupons.show', [$coupon->id]) }}">{{ $coupon->name }}</a></td>
                ...
                <td>
                    <a href="{{ route('provider.coupons.edit', [$coupon->id]) }}"
                        class='btn btn-info btn-sm'>
                            <i class="ti-pencil"></i>
                        </a>
                    {!! Form::open(['route' => ['provider.coupons.destroy', $coupon->id], 'method' => 'delete', 'class' => 'd-inline']) !!}
