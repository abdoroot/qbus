    ...
    <table id="buses-table" class="table display table-bordered table-striped no-wrap">
        <thead>
            <tr>
                ...
                <th>@lang('models/buses.fields.provider_id')</th>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $bus->id }}</td>
                <td>
                    <a href="{{ route('admin.buses.show', $bus->id) }}">{{ $bus->plate }}</a>
                </td>
                <td><img src="{{ asset('images/buses/'.$bus->image) }}" /></td>
                <td>{{ $bus->passengers }}</td>
                <td>
                    @if(!is_null($provider = $bus->provider))
                ...
                <td>
                    ...
                    <a href="{{ route('admin.accounts.show', $account->id) }}">{{ $account->username }}</a>
                    @endif
                </td>
            </tr>
            @endforeach
