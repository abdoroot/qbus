    ...
    <table id="orders-table" class="table display table-bordered table-striped no-wrap">
        <thead>
            <tr>
                ...
                @if(is_null($provider_id))<th>@lang('models/busOrders.fields.provider_id')</th>@endif
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $order->total }}</td>
                <td>
                    @if(!is_null($user = $order->user))
                    <a href="{{ route('admin.users.show', $user->id) }}">{{ $user->name }}</a>
                    @endif
                </td>
                @if(is_null($provider_id))
                <td>
                    @if(!is_null($provider = $order->provider))
                ...
                <td>
                    <a href="{{ route('admin.'.$model.'.show', [$order->id]) }}"
